import React__default, { createContext, useState, useEffect, useContext, Children, cloneElement } from 'react';
...
const FirebaseProvider = ({ children, firebaseConfig }) => {
    if (!firebase$1.apps.length) {
        firebase$1.initializeApp(firebaseConfig);
    }
    return (React__default.createElement(FirebaseContext.Provider, { value: firebase$1 }, children));
};
...
var oboeNode = createCommonjsModule(function (module, exports) {
/*!
 * v2.1.4-104-gc868b3a
 * 
 */
...
const JoinGame = ({ playerName, setPlayerName, setGameId: setGame }) => {
    ...
    return (React__default.createElement(React__default.Fragment, null,
        React__default.createElement("input", { type: "text", placeholder: "Your name", value: playerName, onChange: (e) => setPlayerName(e.target.value) }),
        React__default.createElement("input", { type: "text", placeholder: "Enter the game code", value: gameId, onChange: (e) => setGameId(e.target.value), onKeyPress: (e) => {
                if (e.key == "Enter") {
                    addGame();
                }
            } }),
        !loading && (React__default.createElement(React__default.Fragment, null,
            React__default.createElement("button", { onClick: () => getRandomCode() }, "Suggest random game code"),
            React__default.createElement("button", { onClick: () => addGame() }, "Play!")))));
};
...
const Lobby = ({ gameState, gameRef, playerName, onStart }) => {
    ...
    if (!gameState.players) {
        return React__default.createElement("players-list", null, " No players yet ");
    }
    return (React__default.createElement("players-list", null,
        React__default.createElement("h2", null, "Waiting to play"),
        Object.entries(gameState.players).map(([key, player]) => (React__default.createElement("label", { key: player.name },
            React__default.createElement("player-name", null,
                player.name,
                " is ",
                player.ready ? "ready" : "not ready"),
            player.name === playerName && (React__default.createElement("button", { onClick: () => toggleReady(key) }, "Toggle if Ready"))))),
        React__default.createElement("button", { onClick: () => startGame(), disabled: Object.values(gameState.players).some((x) => !x.ready) }, "Start Game")));
};

var GameAndLobby = ({ gameId, playerName, children, onStart }) => {
    const firebase = useContext(FirebaseContext);
    ...
    return (React__default.createElement("game-container", null,
        React__default.createElement("game-title", null,
            "Game Id - ",
            gameId,
            " - playing as ",
            playerName,
            " ",
            React__default.createElement("a", { href: "/" }, "New Game")),
